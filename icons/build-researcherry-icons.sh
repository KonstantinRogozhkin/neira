#!/usr/bin/env bash
# Скрипт для сборки иконок Researcherry

set -e

QUALITY="researcherry"
COLOR="orange1"
...
build_darwin_main() {
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/darwin/code.icns" ]]; then
    rsvg-convert -w 655 -h 655 "icons/${QUALITY}/researcherry_cnl.svg" -o "code_logo.png"
...
build_darwin_types() {
  rsvg-convert -w 128 -h 128 "icons/${QUALITY}/researcherry_cnl_w80_b8.svg" -o "code_logo.png"
...
build_linux_main() {
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/linux/code.png" ]]; then
    # Создаем PNG иконку из SVG
    rsvg-convert -w 512 -h 512 "icons/${QUALITY}/researcherry_cnl.svg" -o "${SRC_PREFIX}src/${QUALITY}/resources/linux/code.png"
  fi
...
build_media() {
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg" ]]; then
    cp "icons/${QUALITY}/researcherry_clt.svg" "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg"
...
build_server() {
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/server/favicon.ico" ]]; then
    # Создаем ICO из SVG
    rsvg-convert -w 32 -h 32 "icons/${QUALITY}/researcherry_cnl.svg" -o "temp_32.png"
    rsvg-convert -w 16 -h 16 "icons/${QUALITY}/researcherry_cnl.svg" -o "temp_16.png"
    convert temp_16.png temp_32.png "${SRC_PREFIX}src/${QUALITY}/resources/server/favicon.ico"
    rm temp_16.png temp_32.png
  fi
...
build_windows_main() {
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/win32/code.ico" ]]; then
    # Создаем ICO из SVG
    rsvg-convert -w 256 -h 256 "icons/${QUALITY}/researcherry_cnl.svg" -o "temp_256.png"
    rsvg-convert -w 128 -h 128 "icons/${QUALITY}/researcherry_cnl.svg" -o "temp_128.png"
    rsvg-convert -w 64 -h 64 "icons/${QUALITY}/researcherry_cnl.svg" -o "temp_64.png"
    rsvg-convert -w 32 -h 32 "icons/${QUALITY}/researcherry_cnl.svg" -o "temp_32.png"
    rsvg-convert -w 16 -h 16 "icons/${QUALITY}/researcherry_cnl.svg" -o "temp_16.png"
    convert temp_16.png temp_32.png temp_64.png temp_128.png temp_256.png "${SRC_PREFIX}src/${QUALITY}/resources/win32/code.ico"
    rm temp_16.png temp_32.png temp_64.png temp_128.png temp_256.png
  fi
}
...
build_windows_type() {
  ...
  if [[ ! -f "${FILE_PATH}" ]]; then
    ...
    rsvg-convert -w "${LOGO_SIZE}" -h "${LOGO_SIZE}" "icons/${QUALITY}/researcherry_cnl.svg" -o "code_logo.png"
...
build_windows_types() {
  ...
  rsvg-convert -b "#F5F6F7" -w 64 -h 64 "icons/${QUALITY}/researcherry_cnl.svg" -o "code_logo.png"
...
if [[ "${0}" == "${BASH_SOURCE[0]}" ]]; then
  echo "Building Researcherry icons..."
  ...
  echo "Researcherry icons built successfully!"
fi 